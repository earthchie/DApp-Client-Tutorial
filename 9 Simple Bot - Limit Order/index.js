...
async function app(){
    ...
    provider.on('block', async function(){ // start monitoring price

        const price = await getExchangeRateETHBUSD(provider);
        if(price > 3500){ // if 1 ETH > 3500 BUSD

            provider.off('block'); // stop monitoring price
            await sellAllETH(wallet); // swap all ETH in wallet for BUSD
...
async function getExchangeRateETHBUSD(provider){

    // instance pair contract.
    // see chapter 6 for how to find pair address
    const PairContract = new ethers.Contract('0x6a9D99Db0bD537f3aC57cBC316A9DD8b11A703aC', [
        'function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast)'
    ], provider);

    const reserves = await PairContract.getReserves(); // get reserves of tokens
    const price = reserves[0]/reserves[1]; // price = ETH reserves / BUSD reserves

    return price;
}

async function sellAllETH(wallet){
    ...
    if(ETHAmountToPay > 0){    
        const estimateBUSDReceive = await Router.getAmountsOut(ETHAmountToPay, [ETH.address, BUSD.address]); // estimate BUSD recieved from spending all ETH
        const minBUSDReceive = parseInt(estimateBUSDReceive[1] - estimateBUSDReceive[1] * slippage / 100); // set minimum BUSD received
